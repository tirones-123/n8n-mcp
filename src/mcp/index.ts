...
async function main() {
  ...
  try {
    ...
    if (mode === 'http') {
      // Check if we should use SSE implementation
      if (process.env.USE_SSE === 'true') {
        // Use the SSE-enabled HTTP server for Claude API
        const { startSSEHTTPServer } = await import('../http-server-sse');
        await startSSEHTTPServer();
